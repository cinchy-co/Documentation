...
- The **Load Metadata** button now connects you directly to the metadata columns.
- Under the **Schema** section, all columns are now collapsed by default.
...
We've fixed a bug that was stripping query parameters from Relative URLs if they were being used as the Application URL for applets. In the below screenshot, the bug would have stripped out the "q=1" parameter, leaving only an Absolute URL in lieu of a Relative one.
