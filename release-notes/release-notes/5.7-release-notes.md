...
We optimized PostgreSQL queries to be more performant when referencing multi-select columns.
...
- We fixed an issue where the use of a CASE statement on a Link reference with a condition caused an incorrect query.
- We resolved an issue that prevented table exports from reaching the maximum limit of 250,000 records.
