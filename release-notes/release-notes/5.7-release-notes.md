...
**CIN-04672 Remove load metadata modal and execute the action using details provided**
...
We've fixed a bug where if you passed a parameter unencrypted values during a data sync, it would throw an error that would cause the sync to fail. These parameters now work with either encrypted or plaintext values. 