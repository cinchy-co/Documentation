...
The Connections interface has undergone several enhancements to improve user experience and clarity:
...
- Expanded the width of source, destination, and connections drop-down menus to ensure visibility, even on screens with varying sizes.
...
To facilitate sharing and collaboration on connections, we've introduced unique endpoints for all saved connections. Each connection now possesses a unique URL that can be shared with other platform users. This URL links directly to the saved configuration.
...
- The **Load Metadata** modal no longer appears when selecting a relevant source or destination.
...
We've introduced the ability to dismiss modals using the Escape key. This enhancement provides a more convenient and user-friendly interaction experience.
