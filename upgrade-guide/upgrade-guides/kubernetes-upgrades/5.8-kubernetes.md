...
{% hint style="warning" %}
This section only applies if you are upgrading from v5.6 or lower.
{% endhint %}
...
1.  Execute the following command to deploy ArgoCD:
    ...
    ```
1. Validate ArgoCD pods are running and check that ArgoCD is accessible from its portal.

#### Upgrading from 5.6 and lower

The following steps apply only if you are upgrading from Cinchy 5.6 or lower.
