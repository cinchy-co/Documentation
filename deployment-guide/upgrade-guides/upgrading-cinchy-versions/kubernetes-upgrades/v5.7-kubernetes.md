...
After the CRDs are deleted then `redeploy logging-operator` and `kube-prometheus-stack` app with below command. Change directory to **cinchy.argocd** repository:
...
You may see a couple of ArgoCD apps out of sync that are because you had deleted the logging operator. Please sync them manually.
...
1. Execute the following command to deploy ArgoCD:
   ...
   ```
1. Execute the following command from the **cinchy.argocd** repository:
1. Validate ArgoCD pods are running and check that ArgoCD is upgraded v2.7.6 by accessing the ArgoCD application Console. Redis will take a few minutes to recover.
1. Execute the following command to deploy ArgoCD:
...
1. Log in to your ArgoCD application console and refresh the apps to confirm all changes.

### Upgrade AWS EKS and AKS

To upgrade the AWS EKS and Azure AKS version from 1.24 up to 1.27.x, you have two methods. The method depends on the status of the subnet CIDR range. The CIDR is a blocker for Azure only.
...
Make sure you are connected to the appropriate cluster. Before you start the upgrade process of AKS, you must delete all your apps from ArgoCD which will delete Cinchy apps and custom components from ArgoCD, which includes `load balancer` as well `kafka-cluster (Warning)`.
...
1. From terminal change directory to **cinchy.argocd** and run the commands below sequentially. Make sure to change your cluster(directory) name and environment name in below commands:
    ...
    kubectl delete -k environment_kustomizations/hub-nonprod/ENVIRONMENT_NAME/cinchy
    kubectl delete -k argocd
    ```
1. Verify the deletion of pods from that specific namespace with the `kubectl get pods -A` command. 
1. If cluster components deletion takes longer than 10 minutes, run the command below. Check all the pods except `Kubernetes default namespace(kube-system)` pods.
    ```bash
...
1. Launch a shell/terminal with the working directory set to the cluster directory within the cinchy.terraform repo and run `az login`.
